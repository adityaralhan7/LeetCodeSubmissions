class Solution{
    public int minimumPairRemoval(int[] nums){
        ...
        while(unsorted>0&&!pq.isEmpty()){
            ...
            if(val[i]>val[j]) unsorted--;

            int prev=left[i];
            int next=right[j];

            if(prev!=-1&&val[prev]>val[i]) unsorted--;
            if(next!=n&&val[j]>val[next]) unsorted--;
            ...
            if(prev!=-1){
                if(val[prev]>val[i]) unsorted++;
                pq.offer(new long[]{val[prev]+val[i],prev});
            }

            if(next!=n){
                if(val[i]>val[next]) unsorted++;
