class Solution{
    ...
    boolean checkMagic(int row,int col,int side){
        ...
        for(int i=row;i<row+side;i++){
            if(rowPref[i][col+side]-rowPref[i][col]!=target)return false;
        }

        for(int j=col;j<col+side;j++){
            if(colPref[row+side][j]-colPref[row][j]!=target)return false;
        }

        if(diag1Pref[row+side][col+side]-diag1Pref[row][col]!=target)return false;
        if(diag2Pref[row+side][col]-diag2Pref[row][col+side]!=target)return false;
